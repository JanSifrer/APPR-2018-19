...
ggplot(data=meddrzavne.selitve.drzavljanstvo %>% group_by(vrsta, drzava)
  ...
  ggtitle("Graf števila izseljenih in priseljenih po državah") + coord_flip()
```
...
```{r, echo=FALSE, fig.align='center', fig.cap='Zemljevid občin Slovenije'}
ggplot() + geom_polygon(data=left_join(zemljevid, razlika.obcine, by=c("OB_UIME"="obcina")),
                        aes(x=long, y=lat, group=group, fill=razlika)) +
  ggtitle("Razlika priseljenih in odseljenih deljena s številom prebivalcev") + xlab("") + ylab("") +
